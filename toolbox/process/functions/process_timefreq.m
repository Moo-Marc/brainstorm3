...
function sProcess = GetDescription() %#ok<DEFNU>
    ...
    % Options: Normalize
    sProcess.options.labelscale.Comment = '<BR>Wavelet scaling:';
    sProcess.options.labelscale.Type    = 'label';
    sProcess.options.scaling.Comment = {'Decreasing scale - roughly maintains total power', ...
                                        'Increasing scale - roughly maintains relative peak power'; ...
                                        'total', 'peak'};
    sProcess.options.scaling.Type    = 'radio_label';
    sProcess.options.scaling.Value   = 'total';
    sProcess.options.labelnorm.Comment = '<BR>Spectrum normalization:';
    sProcess.options.labelnorm.Type    = 'label';
    sProcess.options.normalize.Comment = {'<B>None</B>: Save non-standardized time-frequency maps', ...
                                          '<B>1/f compensation</B>: Multiply output power values by frequency'; ...
                                          'none', 'multiply2020'};
